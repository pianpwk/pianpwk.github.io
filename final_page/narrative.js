...
// CUSTOM DEFINED
// returns dashed-line if link is not consecutive
narrative.link_dash = function(n) {
	function link_dash(d) {
		if (d.source.scene && d.source.scene.start != d.target.scene.start - 1) {
			return n
		} else {
			return 0
		}
...
// CUSTOM DEFINED
// returns transparent line if link is not consecutive
narrative.link_opacity = function(o1, o2) {
	function link_opacity(d) {
		if (d.source.scene && d.source.scene.start != d.target.scene.start - 1) {
			return o2
		} else {
			return o1
...
narrative.link = function(height, width, y_eps) {
	var curvature = 0.5;
	...
	// // make matrix
	// var intervals = [];
	// for (var i of Array(height-yt).keys()) {
	// 	var temp = [];
	// 	for (var j of Array(width).keys()) {
	// 		temp.push(0)
	// 	}
	// 	intervals.push(temp)
	// }
	...
	function link(d) {
		...
		if (d.source.scene && d.source.scene.start != d.target.scene.start - 1) {
			// xt0 = parseInt(x0+alpha)
			// xt1 = parseInt(x1-alpha)

			var n_exit = 0;
			var n_enter = 0;
			var last_scene = d.source.scene;
			var scene = scenes[last_scene.start];

			var lastsceneNames = new Set(last_scene.characters.map(c => c.id));
			var sceneNames = new Set(scene.characters.map(c => c.id));
			last_scene.characters.forEach(function (c) {
				if (!sceneNames.has(c.id)) { n_exit += 1 }
			})
			scene.characters.forEach(function (c) {
				if (c.appearances[0].scene.start !== scene.start && !lastsceneNames.has(c.id)) { n_enter += 1 }
			})

			var xt0 = x0 + pad_x + dw * exits[d.source.scene.start];
			var xt1 = x1 - pad_x - dw * enters[d.target.scene.start];
			var yt = baseline;
			// while (true) {
			// 	var intersect = false;
			// 	for (var i of intervals) {
			// 		if (i[2] == yt && (i[0] <= xt1 || i[1] >= xt0)) {
			// 			intersect = true;
			// 			break;
			// 		}
			// 	}
			...
			// 	yt += dh;
			// }
			// intervals.push([xt0, xt1, yt])

			// var yt = baseline + dh * exits[last_scene.start];

			exits[d.source.scene.start] += 1;
			enters[d.target.scene.start] += 1;
...
narrative.layout = function() {
	...
	computeScenePositions(alignScenes);
...
function computeScenePositions(alignScenes) {

	var current_x = labelSize[0]
	...
	scenes.forEach(function(scene, i) {
		...
		if (i > 0) {
			var n_exit = 0;
			var n_enter = 0;
			var last_scene = scenes[i-1]

			var lastsceneNames = new Set(last_scene.characters.map(c => c.id));
			var sceneNames = new Set(scene.characters.map(c => c.id));
			last_scene.characters.forEach(function (c) {
				if (c.appearances[c.appearances.length-1].scene.start !== last_scene.start && !sceneNames.has(c.id)) { n_exit += 1 }
			})
			scene.characters.forEach(function (c) {
				if (c.appearances[0].scene.start !== scene.start && !lastsceneNames.has(c.id)) { n_enter += 1 }
			})
		...
		if (orientation === 'vertical') {
			...
			if (i > 0) {
				_x = Math.max(scale, (dw * (n_enter + n_exit)) + (pad_x * 2))
			} else {
				_x = scale
			}

			scene.x = scene._x || Math.max(0, Math.min(size[0], current_x + _x));
	...
	var max_y = 0;
	scenes.forEach(function (scene) {
		scene.appearances.forEach(function (app) {
...
function createIntroductionNodes() {
	...
	appearances.forEach(function(appearance){
		...
		if (orientation === 'vertical') {
			...
			x = appearance.scene.x
			x = appearance.scene.x + 0.5 * labelSize[0];
			y = appearance.y + appearance.scene.y
			// y = appearance.scene.y;
		...
		if (orientation === 'vertical') {
			...
			// introduction.x = Math.max(0, Math.min(size[0]-labelSize[0], x));
			// introduction.y = Math.max(0, Math.min(size[1], y))
		}
